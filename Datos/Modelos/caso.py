...
def crear_caso(tipoCultivo, nombrePlanta, foto, descripcionCaso, estado, evolucionCaso, fechaActualizacion, recomendaciones, idUsuario):
    crear_caso_sql = f"""
        ...
        VALUES ('{tipoCultivo}', '{nombrePlanta}', '{foto}', '{descripcionCaso}', '{estado}', '{evolucionCaso}', '{fechaActualizacion}', '{recomendaciones}', '{idUsuario}')       
    """
    bd = BaseDeDatos()
    bd.ejecutar_sql(crear_caso_sql)

def modificar_caso(idCaso, tipoCultivo, nombrePlanta, foto, descripcionCaso, estado, evolucionCaso, fecha, recomendaciones, especialista):
    modificar_caso_sql = f"""
        UPDATE Caso SET tipoCultivo = '{tipoCultivo}', nombrePlanta = '{nombrePlanta}', foto = '{foto}', descripcionCaso = '{descripcionCaso}', estado = '{estado}', evolucionCaso = '{evolucionCaso}', fechaActualizacion = '{fecha}', recomendaciones = '{recomendaciones}', usuarioEspecialista = '{especialista}'
        WHERE idCaso = '{idCaso}'
    """
    bd = BaseDeDatos()
...
def mostrar_casos_usuario(idUsuario):
    mostrar_caso_por_sql = f"""
        SELECT * FROM Caso WHERE idUsuario = '{idUsuario}'
    """
    bd = BaseDeDatos()
...
def registrar_recomendacion(idCaso, recomendaciones, estado, fechaActualizacion):
    registrar_recomendacion_sql = f"""
        UPDATE Caso SET recomendaciones = '{recomendaciones}', estado= '{estado}', fechaActualizacion={fechaActualizacion}'
        WHERE idCaso= '{idCaso}'
    """
    bd = BaseDeDatos()
